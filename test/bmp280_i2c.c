...
void bmp280_read_raw(int32_t* temp, int32_t* pressure, int32_t* hum) {
...
float compensateHumidity(int32_t adc_H, struct bmp280_calib_param*  cal, int32_t t_fine) 
{
...
int bmp280_main() {
    ...
    while (1) {
        bmp280_read_raw(&raw_temperature, &raw_pressure,&raw_hum);
        int32_t temperature = bmp280_convert_temp(raw_temperature, &params);
        int32_t pressure = bmp280_convert_pressure(raw_pressure, raw_temperature, &params);
 
        int32_t t_fine = bmp280_convert(raw_temperature, &params);
        float hum = compensateHumidity(raw_hum, &params,t_fine);
